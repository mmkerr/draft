---
title: "Local forecasting with National Syndromic Surveillance Program (NSSP) Data"
date: last-modified
format: 
  html: 
    code-overflow: wrap
...
The following plot shows reporting counties as of Jan. 12, 2025. Recall all counties in an HSA will have the same value as the parent HSA. 

```{r}
nssp_county <- covidcast_signal("nssp", "pct_ed_visits_influenza",
                                geo_type = "county",
                                time_type = "week")
```
...
The following code returns county-level data, where `geo_value` is the FIPS code of the county. Again, the values returned for an individual county are really those of the parent HSA.

```{r, message = FALSE}
pub_covidcast(source = "nssp", signals = "pct_ed_visits_influenza", geo_type = "county", geo_values = "25015", time_type = "week")
```
...
The PDF below provides a more detailed breakdown of HSAs for each state. The line plots for each state show the difference between the percentage of emergency department visits for the state (grey line) vs. each individual HSA in that state (orange line). 


{{< pdf all_states_faceted.pdf width=120% height=800 >}}
